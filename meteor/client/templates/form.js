...
checkboxes = function(record) {
  ...
  cb.append("text").text(function(d) {
    return d;
  });
...
Template.form.onRendered(function() {
  ...
  $(".knobVal").html(degree);
  if (
    !record.incidentLocation.ecoregionDomain ||
    !record.incidentLocation.ecoregionDivision
  ) {
  ...
  $('[name="findLocation.distanceIPP"]')
    .prev()
    .append(" (" + labelUnits(currentUnit, "distance") + ")");
  $('[data-subjecttable="Weight"]').append(
    " (" + labelUnits(currentUnit, "weight") + ")"
  );
  $('[data-subjecttable="Height"]').append(
    " (" + labelUnits(currentUnit, "height") + ")"
  );
  if (
    $('[name="incidentOutcome.incidentOutcome"]').val() !== "Open/Suspended"
  ) {
...
Template.form.helpers({
  ...
  initialDirectionofTravelClass: function() {
    var initialDirectionofTravel_Boolean =
      this.incidentOperations.initialDirectionofTravel_Boolean === "Yes"
        ? true
        : false;
    Session.set(
...
Template.form.events({
  ...
  "change .bsDateInput": function(event, template) {
    ...
    if (
      name === "timeLog.lastSeenDateTime" ||
      name === "timeLog.subjectLocatedDateTime" ||
      name === "timeLog.SARNotifiedDatetime"
    ) {
      ...
      if (startTimeSAR && endTime) {
        var duration = moment.duration(
          moment(endTime).diff(moment(startTimeSAR))
        );
        var elapsedTime = Math.round(duration.asHours());
        Meteor.call(
          "updateRecord",
  ...
  "click .mapPoints .btn": function(event, template) {
    ...
    if (!active) {
      ...
      if (
        confirm(
          "Are you sure you want to remove " + item.text + " from the map?"
        )
      ) {
        map.remove(item);
      } else {
    ...
    Meteor.call("setWeather", id, function(err, d) {
      if (err) {
        $(event.target).replaceWith(
          '<p class="small em mar0y text-danger">Unable to retreive weather data</p>'
        );
        return console.log(err);
      } else {
      }
  ...
  "click #mapCalcBtn": function(event, template) {
    ...
    Meteor.call("setBearing", recordId, "findLocation.findBearing", function(
      err,
      d
    ) {
      if (err) {
  ...
  'change [name="incidentOperations.initialDirectionofTravel_Boolean"]': function(
    event
  ) {
    ...
    if (val) {
      ...
      Meteor.call(
        "updateRecord",
        record._id,
        "incidentOperations.initialDirectionofTravel",
        null,
        function(err, d) {
          if (err) {
            return console.log(err);
  ...
  if (modifier && modifier.$set && Object.keys(modifier.$set).length) {
    ...
    _.each(fields, function(d) {
      if (_.contains(hide, d.field)) {
        modifier.$set[d.name] = CryptoJS.AES.encrypt(
          modifier.$set[d.name],
          encryptionKey
        ).toString();
      }
...
formSetMap = function(context, recordId) {
  ...
  obj.add = function(d) {
    ...
    if (val === "intendedRoute" || val === "actualRoute") {
      ...
      var latlngs = [[start.lat, start.lng], [end.lat, end.lng]];
  ...
  obj.addPoly = function(d, latlngs) {
    ...
    if (!$('[name="' + d.name + '"]').val()) {
      Meteor.call("updateRecord", recordId, d.name, lineString, function(
        err,
        d
      ) {
        if (err) {
    ...
    $("#formMap").on("mouseup", ".leaflet-editing-icon", function(d) {
      drawnPaths.eachLayer(function(layer) {
        ...
        if (layer._path) {
          latlngs = layer.getLatLngs().map(function(d) {
            return [d.lat, d.lng];
          });
          var lineString = JSON.stringify(latlngs);
          $('[name="' + name + '"]')
  ...
  obj.addPoint = function(d) {
    ...
    var draggable = Roles.userIsInRole(Meteor.userId(), ["editor", "admin"])
      ? true
      : false;
    ...
    marker.on("dragend", function(event) {
      ...
      Meteor.call("updateRecord", recordId, d.name, position, function(
        err,
        res
      ) {
        if (err) {
  ...
  obj.fitBounds = function() {
    map.fitBounds(drawnPoints.getBounds().pad(0.5));
