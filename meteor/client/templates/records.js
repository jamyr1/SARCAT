...
Template.records.onRendered(function() {
  if (!Session.get("getRecords")) {
    Session.set("getRecords", new Date());
  }
  ...
  $('label:contains("Subject Category")').after(
    '<span class="small em mar0y text-default"><a class="em" href="/profiles" target="_blank"}}"> *info</span>'
  );
});
  ...
  settings: function() {
    ...
    if (Roles.userIsInRole(Meteor.userId(), ["admin", "editor"])) {
      fields.unshift({
        ...
        label: function() {
          return new Spacebars.SafeString(
            '<input type="checkbox" class="recordSelAll">'
          );
        },
...
Template.records.events({
  ...
  "click .deleteRecord": function() {
    ...
    var names = toDelete.map(function(d) {
    ...
    if (confirm(message)) {
      Meteor.call("removeRecord", toDelete, function(error) {
        ...
        records = records.filter(function(d) {
          return !toDelete.some(function(e) {
            return e === d._id;
          });
        });
  ...
  'blur [name="coords.ippCoordinates.lat"],[name="coords.ippCoordinates.lng"]': function(
    event,
    template
  ) {
  ...
  "click .uploadISRID": function() {
    var checked = $(".recordSel:checked")
      ...
      .toArray();
    if (!checked.length) {
      checked = Session.get("records").map(function(d) {
        return d._id;
      });
    }

    var records = Session.get("records") || [];
    var allRecords = records.filter(function(d) {
      return checked.indexOf(d._id) !== -1;
    });
  ...
  "click #genReport": function() {
    ...
    if (!checked.length) {
      checked = allRecords.map(function(d) {
        return d._id;
      });
    }
    ...
    Session.set("userAlert", {
      error: false,
      text: "Generating Report for " + checkLen + " of " + allLen + " Records"
    });
    setTimeout(function() {
      Router.go("report", {
        ids: checked.join(".")
  ...
  "click #downloadRecords": function() {
    ...
    JSONToCSVConvertor(
      allRecordsFlat,
      "sarcatExport-" + moment().format("MMMM-Do-YYYY_h-mm-ssa")
    );
  },
  ...
  "click #downloadRecordsJSON": function() {
    ...
    var JSONConvertor = function(JSONData, ReportTitle) {
      ...
      var uri =
        "data:text/csv;charset=utf-8," + escape(JSON.stringify(JSONData));
      var link = document.createElement("a");
      link.href = uri;
      link.style = "visibility:hidden";
