Meteor.methods({
  getAudits: function() {
    var arr = RecordsAudit.find({}, {}).fetch();
    //return arr;
    return arr
      .sort(function(d, e) {
        var date1 = d.date;
  ...
  getFilesInBackupFolder: function() {
    var fs = Npm.require("fs");
    var path = Npm.require("path");
    var myPath = fs.realpathSync(".");
    var filePath = path.join(myPath, "assets", "app", "backup");
    //var dir = '../web.browser/app/uploads/records/' + id;
    if (!fs.existsSync(filePath)) {
      return [];
    }
    var files = fs.readdirSync(filePath);
    /*Assets.getText('backup/sample.json', function(d, e) {
            console.log(d, e)
        });*/
    return files;
  },
  backupJSON: function(name, json) {
    console.log(name);
    fs = Npm.require("fs");
    path = Npm.require("path");
    var myPath = fs.realpathSync(".");
    var filePath = path.join(myPath, "assets", "app", "backup", name + ".json");
  ...
  encryptionKey: function(set, userId) {
    ...
    return Config.findOne().encryptionKey;
  },
  ...
  pushArray: function(id, name, val) {
    var obj = {};
    obj[name] = val || {};
  ...
  pullArray: function(id, name, val) {
    var obj = {};
    obj[name] = val || {};
    console.log(obj);
    var update = Records.update(id, {
  ...
  removeSubject: function(recordId, subjectId) {
    var newSubjects = Records.findOne(recordId).subjects.subject.filter(
      function(d) {
  ...
  removeResource: function(recordId, resourceId) {
    var newResource = Records.findOne(recordId).resourcesUsed.resource.filter(
      function(d) {
  ...
  getFilesInPublicFolder: function(id) {
    var fs = Npm.require("fs");
    var path = Npm.require("path");
    var dir = process.env.sarcatStorage
      ? process.env.sarcatStorage
      : process.env.PWD;
  ...
  setEcoRegion: function(id) {
    ...
    var ecoregionDivision =
      val.DIV_NUM && val.DIV_DESC
        ? val.DIV_NUM + "-" + val.DIV_DESC
        : "Unknown";
    Records.update(id, {
      $set: {
  ...
  setWeather: function(id, options) {
    var options = options || {};
    var internet = Config.findOne().internet;
    if (!internet) {
      return;
    }
    var record = Records.findOne(id);
  ...
  setDispersionAngle: function(id) {
    ...
    var initialDirectionofTravel =
      record.incidentOperations.initialDirectionofTravel;
